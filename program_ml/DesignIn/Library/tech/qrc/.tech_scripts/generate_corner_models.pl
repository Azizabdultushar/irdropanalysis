...
printf outf "`perl \$script_dir/mw_3d.pl 5 \$profile_dir \$model_dir \$techfile \$version -exe $exe_dir`;\n\n";

#write sensitivity models
if ($opt_sensitivity) {
  printf outf "\n \#\n\# Sensitivity Model writer section\n\#\n";
  printf outf "`perl %s/mw_3d.pl 5 -sensitivity $sens_file \$profile_dir \$model_dir \$techfile \$version -exe $exe_dir`;\n\n", $script_dir;
}
close(outf);
