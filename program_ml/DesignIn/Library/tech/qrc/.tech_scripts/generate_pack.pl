...
printf outf "`perl \$script_dir/mw_limits.pl %d \$model_dir \$profile_dir \$version -exe $exe_dir`;\n\n", $extension;
...
if ($exe_dir eq "") {
  ...
  printf outf "\$model_pruner = \"$exe_dir/modelgen prune \";\n\n";
}
...
if ($opt_sensitivity) {
  ...
  printf outf "`perl \$script_dir/mw_limits.pl %d \$sens_model_dir \$sens_profile_dir \$version -sensitivity \$sens_file -script_dir \$script_dir -exe $exe_dir`;\n\n", $extension;
